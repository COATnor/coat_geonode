...
class MothFileUploadTests(APITestCase):
    ...
    def rest_upload_file(self):
        ...
        self.assertTrue(urlparse(response.data['datafile']).path.strtswith(settings.MEDIA_URL))
        self.assertEqual(response.data['user'], User.objects.get_by_natural_key('test').id)
        self.assertIn('created', response.data)

        #assert unauthenticated user cannnot upload file
        client.logout()
        response = client.post(url, data, format='multipart')
        self.assertEqual(response.status_code, status.HTTP_401_UNHAUTORIZED)